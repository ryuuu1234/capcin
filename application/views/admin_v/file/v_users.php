							...
							<div class="modal-body"> 
								<form id="add-form" action="#" class="form-horizontal" role="form">
									...
									<div class="row">
										...
										<?=textInput('username','off','10')?>
										<?=textInput('email','off','10')?>
										<?=selectInput('level','10', $level)?>
...
$(document).on('click', '.btnConfirmed', function(){
	...
	$.ajax({
                    url: '<?=base_url()?>admin/file/users/konfirm_users', // url to wich the request is send
...
{   
    $('#gambar_input_'+id+':hidden').trigger('click');
    $('#gambar_input_'+id).change(function(e) {
            $('#msg-img-upload').empty();
            var file =  this.files[0];
            var imagefile = file.type;
            var match= ['image/jpeg','image/png','image/jpg'];
            if(!((imagefile==match[0]) || (imagefile==match[1]) || (imagefile==match[2]))) 
            {
                // $('#preview-image').attr('src', '<?=base_url();?>assets/images/big/2157fa42e60250d.png');
                $('#msg-img-upload').html("<p id='error'>Extensi Gambar tidak Valid</p>"+"<h4>Catatan</h4>"+"<span id='error_message'><p>file salah, ulangi</p></span>");
                return false;
            }
            else
            {
                var reader = new FileReader();
                reader.onload = imgLoginUpload;
                reader.readAsDataURL(this.files[0]);

            }
        });
    function imgLoginUpload(e) {

            $('#preview-image-'+id).attr('src', e.target.result);
            $('#preview-image-'+id).attr('class', 'img-responsive thumbnail');
                // $("#btnGrup-"+id).show();
                $("#form-image-"+id).trigger('submit');
        };

        // $(document).ready(function(e) {
           $('#form-image-'+id).on('submit', (function(e) {
                e.preventDefault();
                $('#msg-img-upload-'+id).empty();
                $('#loading-img-upload-'+id).show();
                var action_page = $('#action_page_'+id).val();
                var page = $('#upload-link').attr('data-page');
                
                $.ajax({
                    url: action_page + id, // url to wich the request is send
                    type: "POST",               // type to request
                    ...
                    {   
                        ...
                        console.log(resp);
                        $('#loading-img-upload-'+id).hide();
                        $('#btnSuksesLogin-'+id).html('Tersimpan.');
                        $('#btnSuksesLogin-'+id).fadeOut(3000);
                         $('#gambar_input').val('');
                         showList('goto');
                    }
                });
